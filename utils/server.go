...
type MonitorSearchRes map[string]interface{}

func ServerInfo() MonitorSearchRes {
	...
	if err == nil {
		...
		if err == nil {
			goUsed = memInfo.RSS
			goUsage, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", float64(goUsed)/float64(memTotal)*100), 64)
		}
	}
	...
	return MonitorSearchRes{
		"cpuNum":          cpuNum,
		"cpuUsed":         cpuUsed,
		"cpuAvg5":         fmt.Sprintf("%.2f", cpuAvg5),
		"cpuAvg15":        fmt.Sprintf("%.2f", cpuAvg15),
		"memTotal":        memTotal,
		"goTotal":         goTotal,
		"memUsed":         memUsed,
		"goUsed":          goUsed,
