...
func (c NacosConfig) getConfig() ([]constant.ServerConfig, *constant.ClientConfig) {
	return c.getServerConfig(), c.getClientConfig()
}

func (c NacosConfig) getClientConfig() *constant.ClientConfig {
...
func (c NacosConfig) getServerConfig() []constant.ServerConfig {
...
type TLSConfig struct {
	Appointed          bool   // 是否指定，如果为false，将从环境变量获取
	Enable             bool   // 启用TLS
	TrustAll           bool   // 信任所有服务器
	CaFile             string // 客户端验证服务器证书时使用
	CertFile           string // 服务器验证客户端证书时使用
	KeyFile            string // 服务器验证客户端证书时使用
	ServerNameOverride string // 仅用于测试的服务器名称覆盖
}
